...
include 'koneksi.php';
$output='';
$query = "SELECT * FROM komentar WHERE parent_id = '0' ORDER BY id_komentar DESC";
    ...
    <div class="media border p-3 mb-2">
      ...
      <div class="row">
        ...
        <div class="col-sm-2" align="right">
          <button type="button" class="btn btn-primary reply" id="'.$row["id_komentar"].'">Reply</button>
        </div>
      </div>
      </div>
    </div>
  ';
  $output .= ambil_reply($db1, $row["id_komentar"]);
}
...
{
  $output='';
  $query = "SELECT * FROM komentar WHERE parent_id=?";
  $dewan1 = $db1->prepare($query);
  ...
  if($count > 0){
    while ($row = $res1->fetch_assoc()) {
      ...
      ';
      $output .= ambil_reply($db1, $row["id_komentar"], $marginleft);
    }
  }
  return $output;
}
